  ...
  >
    ...
    <div class="vol-container">
      <div class="vol-header">
        <!-- 这里可以放项目名称 -->
        <div class="project-name">项目管理平台</div>
...
// 修改菜单初始化方法
const initMenuWithCustomTab = () => {
  inintMenu(proxy, dataConfig, router, onSelect)
  
  // 在MES菜单前插入
  const mesIndex = menuData.value.findIndex(item => 
    item.name && item.name.includes('MES')
  )
  if (mesIndex > -1) {
    menuData.value.splice(mesIndex, 0, customTab)
  }
}

initMenuWithCustomTab()

Object.assign(proxy.$tabs, { 
  open: open, 
  close: close,
  // 添加自定义打开方法
  openCustomTab: () => {
