...
<script setup>
import options from "./SC_ProjectOptions.js";
...
const { proxy } = getCurrentInstance();
	...
	const modelOpenBefore = (row, index, obj, callback) => {
        //跳转到新页面编辑
        uni.navigateTo({
			url: "/pages/sc/SC_Project/SC_ProjectEdit?id=" + ((row || {})[table.key] || ''),
            fail(e) {
                console.log(e)
            }
		})
...
<!--
提示：如有 v-for 警告，请在 vol-table、vol-view 组件内部的 v-for 元素上补充 :key 属性，如 :key="item.id" 或 :key="index"
-->
