...
namespace VOL.Core.Filters
{
    public abstract class ServiceFunFilter<T> where T : class
    {
        ...
        /// </summary>
        /// <remarks>
        /// 警告：当设置此SQL语句时，开发者必须确保SQL的安全性，防止SQL注入攻击。
        /// 最好使用参数化查询或确保所有动态构建的部分都经过严格的清理和验证。
        /// (Warning: When setting this SQL statement, developers must ensure its security to prevent SQL injection attacks.
        /// It is best to use parameterized queries or ensure that all dynamically constructed parts are strictly sanitized and validated.)
        /// </remarks>
        protected string? QuerySql = null; // // 标记为可空，并添加安全警告
        ...
        /// </summary>
        protected Expression<Func<T, Dictionary<object, QueryOrderBy>>>? OrderByExpression; // // 标记为可空

        /// <summary>
        /// 设置查询的表名(已弃用)
        /// </summary>
        protected string? TableName { get; set; } // // 标记为可空

        /// <summary>
        /// 页面查询或导出，从数据库查询出来的结果
        /// </summary>
        protected Action<PageGridData<T>>? GetPageDataOnExecuted; // // 标记为可空

        /// <summary>
        /// 调用新建处理前(SaveModel为传入的原生数据)
        /// </summary>
        protected Func<SaveModel, WebResponseContent>? AddOnExecute; // // 标记为可空

        /// <summary>
        /// 调用新建保存数据库前处理(已将提交的原生数据转换成了对象)
        ///  Func<T, object,ResponseData>  T为主表数据，object为明细数据(如果需要使用明细对象,请用 object as List<T>转换) 
        /// </summary>
        protected Func<T, object?, WebResponseContent>? AddOnExecuting; // // object可以为null，整个委托也可空
        ...
        /// </summary>
        protected Func<T, object?, WebResponseContent>? AddOnExecuted; // // object可以为null，整个委托也可空

        /// <summary>
        /// 进入审批流程方法之前
        /// </summary>
        protected Func<T, bool>? AddWorkFlowExecuting; // // 标记为可空

        /// <summary>
        /// 写入审批流程数据之后
        /// list:审批的人id
        /// </summary>
        protected Action<T,List<int>>? AddWorkFlowExecuted; // // 标记为可空

        /// <summary>
        /// 调用更新方法前处理(SaveModel为传入的原生数据)
        /// </summary>
        protected Func<SaveModel, WebResponseContent>? UpdateOnExecute; // // 标记为可空
        ...
        /// </summary>
        protected Func<T, object?, object?, List<object>?, WebResponseContent>? UpdateOnExecuting; // // object们和List<object>都可以为null，整个委托也可空
        ...
        /// 如果执行的是手写sql请用repository.DbContext.Database.ExecuteSqlCommand()或 repository.DbContext.Set<T>().FromSql执行具体sql语句
        /// </summary>
        protected Func<T, object?, object?, List<object>?, WebResponseContent>? UpdateOnExecuted; // // object们和List<object>都可以为null，整个委托也可空

        /// <summary>
        /// 删除前处理,object[]准备删除的主键
        /// </summary>
        protected Func<object[], WebResponseContent>? DelOnExecuting; // // 标记为可空

        /// <summary>
        /// 删除后处理,object[]已删除的主键,此处已开启了DbContext事务(重点),如果还有其他业务处事，直接在这里写EF代码,不需要再开启事务
        /// 如果执行的是手写sql请用repository.DbContext.Database.ExecuteSqlCommand()或 repository.DbContext.Set<T>().FromSql执行具体sql语句
        /// </summary>
        protected Func<object[], WebResponseContent>? DelOnExecuted; // // 标记为可空

        /// <summary>
        /// 审核前处理
        /// </summary>
        protected Func<List<T>, WebResponseContent>? AuditOnExecuting; // // 标记为可空
        /// <summary>
        /// 审核后处理
        /// </summary>
        protected Func<List<T>, WebResponseContent>? AuditOnExecuted; // // 标记为可空
        ...
        /// bool:当前数据是否为最后一个人审核
        /// </summary>
        protected Func<T, AuditStatus, bool, WebResponseContent>? AuditWorkFlowExecuting; // // 标记为可空
        ...
        /// bool:当前数据是否为最后一个人审核
        /// </summary>
        protected Func<T, AuditStatus,List<int>, bool, WebResponseContent>? AuditWorkFlowExecuted; // // 标记为可空
        ...
        /// </summary>
        protected Func<List<T>, List<string>, WebResponseContent>? ExportOnExecuting; // // 标记为可空

        /// <summary>
        /// 2020.05.07
        /// 导出表数据(界面上导出操作),指定要导出的列，格式:Expression<Func<T, object>> exp = x => new { x.字段1, x.字段2 }
        /// </summary>
        protected Expression<Func<T, object>>? ExportColumns { get; set; } // // 标记为可空
        /// <summary>
        /// 指定要导出的列
        /// </summary>
        protected string[]? ExportColumnsArray { get; set; } // // 标记为可空

        /// <summary>
        /// 2020.05.07
        /// 导出下载模板，指定要导出的模板列，格式:Expression<Func<T, object>> exp = x => new { x.字段1, x.字段2 }
        /// </summary>
        protected Expression<Func<T, object>>? DownLoadTemplateColumns { get; set; } // // 标记为可空

 
        /// <summary>
        /// 导入保存后
        /// </summary>
        protected Func<List<T>, WebResponseContent>? ImportOnExecuted; // // 标记为可空

        /// <summary>
        /// 导入保存前
        /// </summary>
        protected Func<List<T>, WebResponseContent>? ImportOnExecuting; // // 标记为可空

        /// <summary>
        /// 导入时不验证下拉框数据源的字段值2023.05.03
        /// </summary>
        protected Expression<Func<T, object>>? ImportIgnoreSelectValidationColumns; // // 标记为可空
        ...
        /// string=返回的值
        /// </summary>
        protected Func<string?, ExcelWorksheet, ExcelRange, int, int, string?>? ImportOnReadCellValue; // // 参数和返回类型都可能为null，整个委托也可空


        /// <summary>
        /// 自定义上传文件夹(2022.10.07)
        /// </summary>
        protected string? UploadFolder = null; // // 标记为可空
